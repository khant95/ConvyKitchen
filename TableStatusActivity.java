...
public class TableStatusActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Tables Status");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        recyclerView.setLayoutManager(new GridLayoutManager(this,2));
        recyclerView.setHasFixedSize(true);
        mAuth=FirebaseAuth.getInstance();
        mUser=mAuth.getCurrentUser();
        mRef=FirebaseDatabase.getInstance().getReference().child("Tables").child(mUser.getUid());
        ...
        if (status)
        {
            ...
            mRef.child(key).child("status").setValue(false).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    ...
                    {
                        Toast.makeText(TableStatusActivity.this, "Reserved!", Toast.LENGTH_SHORT).show();
        ...
        else
        {
            mRef.child(key).child("status").setValue(true).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    ...
                    {
                        adapter.notifyDataSetChanged();
                        Toast.makeText(TableStatusActivity.this, "Available!", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
